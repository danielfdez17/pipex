...
int	ft_cases(char const *av, va_list a_list)
{
	...
	if (*av == 'x' || *av == 'X')
		return (ft_put_hex(va_arg(a_list, unsigned int), *av));
	if (*av == 'p')
		return (ft_put_ptr(va_arg(a_list, unsigned long)));
	if (*av == 'd' || *av == 'i')
...
static void	advance_pointer(char const **av, int *tmp)
{
	(*av)--;
	*tmp = 1;
}
